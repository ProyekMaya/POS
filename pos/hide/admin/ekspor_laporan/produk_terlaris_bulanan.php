...
if (isset($_GET['format']) && $_GET['format'] == 'pdf') {
    ...
    $pdf->Cell(0,10,'Laporan Produk Terlaris Bulanan',0,1,'C');
    ...
    $pdf->Cell(65,10,'Daftar ID penjualan',1,0,'C');
    $pdf->Ln();
    ...
    foreach ($data as $row) {
        ...
        $nama_produk_cell_width = 55;
        $daftar_id_penjualan_cell_width = 65;
    ...
    if (empty($data)) {
        $pdf->Cell(0, 10, 'Tidak ada produk terlaris ditemukan untuk bulan ini.', 1, 1, 'C');
    }
    
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell(65, 10, 'Total Seluruh Produk Terjual (Pcs):', 1, 0, 'R');
    $pdf->Cell(25, 10, number_format($total_seluruh_produk_terjual_pcs), 1, 0, 'C');
    $pdf->Cell(35 + 65, 10, '', 1, 1, 'L');
    $pdf->Cell(90, 10, 'Total Seluruh Produk Terjual (Dus/Ctn):', 1, 0, 'R');
    $pdf->Cell(35, 10, number_format($total_seluruh_produk_terjual_dus_ctn_temp), 1, 0, 'C');
    $pdf->Cell(65, 10, '', 1, 1, 'L');
    
    $pdf->Output('I', 'Produk_Terlaris_'.$bulan.'_'.$tahun.'.pdf');
    exit;
}
?>
...
<html>
<head>
    <title>Produk Terlaris Bulanan</title>
...
<body class="container mt-4">
    <h2>Laporan Produk Terlaris Bulanan</h2>
    <h4>Bulan: <?= $bulanNama ?> Tahun: <?= $tahun ?></h4>

    <a href="produk_terlaris.php?format=pdf&bulan=<?= $bulan ?>&tahun=<?= $tahun ?>" class="btn btn-danger mt-3 mb-3">
        Ekspor ke PDF
    </a>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <?php if (empty($data)): ?>
                <tr>
                    <td colspan="5" class="text-center">Tidak ada produk terlaris ditemukan untuk bulan ini.</td>
                </tr>
            <?php endif; ?>
        </tbody>
        <tfoot>
